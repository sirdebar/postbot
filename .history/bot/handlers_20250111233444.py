...
async def admin_handler(message: Message, command: CommandObject):
    ...
    if not command.args:
        ...
        return
    user = await bot.get_chat(command.args.replace("@", ""))
    ...
    await message.reply(f"Пользователь {user.mention} назначен администратором.")
...
async def deladmin_handler(message: Message, command: CommandObject):
    ...
    await message.reply(f"Пользователь {user.mention} лишен админских прав.")
...
async def my_handler(message: Message, command: CommandObject):
    ...
    response = (
        f"Статистика для {target_user.mention}:\n\n"
