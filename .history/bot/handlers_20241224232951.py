...
from bot.database import (
    add_to_waiting, get_all_records, get_records_by_status, update_status, clear_all, delete_expired_records
)
...
def setup_handlers(dp: Dispatcher):
    ...
    dp.callback_query.register(paginate_list)
...
async def list_all_handler(message: Message):
    await delete_expired_records()  # Очистка устаревших записей
    records = await get_all_records()
    response, keyboard = format_records(records, "Общий список")
